...
detail = "logs/detail/" + datetime.now().strftime("%m-%d-%Y_%H-%M-%S") + ".log"
logging.basicConfig(
    format="%(asctime)s: %(message)s", filename=detail, level=logging.INFO
)
...
headers = {"timestamp": "timestamp", "price": "askprice"}  # the column used for price
...
def getInfo():
    ...
    return "Pair: [{}]<br>\
        ...
        threader.fiat,
        threader.predicting,
        threader.conservative,
        datetime.fromtimestamp(threader.last_time_trained).strftime(
            "%m-%d-%Y %H:%M:%S"
        ),
    )
...
def table():
    ...
    if len(dataset.index) >= 1:
        vals = dataset.iloc[-1].to_frame().T.to_html(table_id="latest", index=False)
        # print(vals)
        top_row = vals
    else:
    ...
    if len(head.values) >= show:
        ...
        head_html = (
            head.iloc[-(len(head_html)) - 1 :].iloc[::-1].to_html(table_id="csv")
        )
